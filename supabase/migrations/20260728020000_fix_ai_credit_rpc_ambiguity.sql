...
begin
  ...
  where usage_event.user_id = v_user_id
  ...
  if v_existing.id is not null and v_existing.status in ('reserved', 'completed') then
    return query
      select v_existing.status, v_existing.credits_used, greatest(0, p_allowance - v_committed);
    return;
  end if;

  if v_reserved >= p_allowance then
    return query
      select 'limit_reached'::text, 0, greatest(0, p_allowance - v_committed);
    return;
  end if;

  if v_existing.id is not null and v_existing.status = 'released' then
    update public.ai_usage_events as usage_event
    set feature = p_feature,
      credits_used = 1,
      status = 'reserved',
    ...
    insert into public.ai_usage_events as usage_event (
      user_id,
      request_id,
      feature,
      credits_used,
  ...
  return query
    select 'reserved'::text, 1, greatest(0, p_allowance - v_committed);
end;
$$;
...
begin
  ...
  set status = 'completed',
    credits_used = 1,
    completed_at = coalesce(usage_event.completed_at, now())
  where usage_event.user_id = v_user_id
    and usage_event.request_id = p_request_id
  ...
  where monthly_usage.user_id = v_user_id
    and monthly_usage.period_start = v_period_start
    and monthly_usage.status = 'completed';

  return query
    select v_event.status, v_event.credits_used, greatest(0, p_allowance - v_completed);
end;
$$;
