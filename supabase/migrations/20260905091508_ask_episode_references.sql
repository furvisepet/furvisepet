begin;
-- No aggregate/backfill or episode regrouping. Existing RLS remains authoritative.
create index ask_episode_topic_start_idx on public.pet_care_episodes
  (user_id,pet_profile_id,normalized_key,started_at,id);
create index ask_episode_member_scope_idx on public.pet_care_entries
  (episode_id,user_id,pet_profile_id,occurred_at,created_at,id);
create index ask_displayed_episode_refs_idx on public.ask_conversation_messages
  (user_id,conversation_id,sequence_number desc)
  where role='furvise' and response_data ? 'episodeReferences';

create function public.read_ask_episode_references(p_conversation_id uuid)
returns jsonb language plpgsql stable security invoker set search_path=pg_catalog as $$
declare result jsonb; timeout_ms numeric;
begin
  if auth.uid() is null or not exists(select 1 from public.ask_conversations
    where id=p_conversation_id and user_id=auth.uid()) then
  ...
  select response_data->'episodeReferences' into result
  from public.ask_conversation_messages where user_id=auth.uid()
    and conversation_id=p_conversation_id and role='furvise'
    and response_data ? 'episodeReferences' order by sequence_number desc limit 1;
  if octet_length(result::text)>12000 then return null; end if;
  return result;
end $$;

create function public.read_ask_episode_sources(p_pet_id uuid,p_keys text[],p_episode_ids uuid[] default null)
returns jsonb language plpgsql stable security invoker set search_path=pg_catalog as $$
    ...
    into episodes,ids from (
      select e.* from (select distinct unnest(p_keys) k) keys cross join lateral (
        ...
        from public.pet_care_episodes where user_id=auth.uid() and pet_profile_id=p_pet_id
          and normalized_key=k order by started_at,id limit 9
...
revoke all on function public.read_ask_episode_references(uuid) from public,anon,service_role;
revoke all on function public.read_ask_episode_sources(uuid,text[],uuid[]) from public,anon,service_role;
grant execute on function public.read_ask_episode_references(uuid) to authenticated;
grant execute on function public.read_ask_episode_sources(uuid,text[],uuid[]) to authenticated;
