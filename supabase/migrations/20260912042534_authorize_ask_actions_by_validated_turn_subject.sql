-- Conversation selection is UI context. Mutation scope is the server-owned
-- subject-integrity decision persisted with this exact assistant turn.
-- Authenticated clients have SELECT only on ask_conversation_messages.
create or replace function private.ask_action_pet_matches_turn(
  p_conversation_pet uuid, p_governance jsonb, p_target_pet uuid
) returns boolean
language sql immutable security invoker set search_path = ''
as $$
  select coalesce(case
    when p_governance #> '{semanticTrace,subjectIntegrity,authoritativePetIds}' is null
      then p_target_pet = p_conversation_pet
    else p_governance #> '{semanticTrace,subjectIntegrity,agrees}' = 'true'::jsonb
      and p_governance #> '{semanticTrace,subjectIntegrity,authoritativePetIds}' = jsonb_build_array(p_target_pet::text)
    end, false);
$$;
revoke all on function private.ask_action_pet_matches_turn(uuid,jsonb,uuid) from public,anon,authenticated;
grant execute on function private.ask_action_pet_matches_turn(uuid,jsonb,uuid) to service_role;
...
begin
  ...
  select p.* into v_pet from public.ask_conversations c
    join public.dog_profiles p on p.id = new.pet_profile_id and p.user_id = c.user_id
    where c.id = v_source.conversation_id and c.user_id = new.user_id
      and private.ask_action_pet_matches_turn(c.pet_profile_id, v_assistant.persistence_governance, new.pet_profile_id);
...
begin
  ...
  -- that the server-validated turn still authorizes the capability's exact pet.
  if not exists (
    select 1 from public.ask_conversation_messages source_message
    ...
    join public.ask_conversations conversation_row
      ...
      and conversation_row.user_id = v.user_id
      and private.ask_action_pet_matches_turn(conversation_row.pet_profile_id, assistant_message.persistence_governance, v.pet_profile_id)
    where source_message.id = v.source_message_id
