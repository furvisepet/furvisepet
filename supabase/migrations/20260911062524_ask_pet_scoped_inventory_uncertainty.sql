-- Preserve uncertainty for the affected pet; never clear a deletion receipt.
-- Legacy owner-only receipts cannot be reconstructed, so conservatively bind
-- them to every existing pet. New pets do not inherit another pet's old debt.
set lock_timeout = '3s';
lock table public.dog_profiles, public.pet_care_entries, public.pet_care_episodes,
 public.pet_care_episode_events, public.semantic_claims, public.semantic_claim_relations,
 public.semantic_claim_legacy_lineage, public.ask_history_removed_relation_targets,
 public.ask_recorded_inventory_removals in share row exclusive mode;
alter table public.ask_recorded_inventory_removals add column affected_pet_ids uuid[] not null default '{}';
alter table public.ask_history_removed_relation_targets add column affected_pet_ids uuid[] not null default '{}';

-- Internal trigger helper. IDs only, no deleted source text is retained.
-- Missing, non-pet or incomplete endpoint identities widen to all current pets.
create function private.ask_removed_pet_scope(p_owner uuid, p_row jsonb) returns uuid[]
language plpgsql stable security definer set search_path=pg_catalog as $$
declare ids uuid[]; direct_id uuid; endpoints uuid[]; endpoint_count integer;
begin
 direct_id := coalesce(p_row->>'pet_profile_id',p_row->>'pet_id',
   case when p_row->>'subject_type'='pet' then p_row->>'subject_id' end)::uuid;
 if direct_id is not null then return array[direct_id]; end if;
 select array_agg(distinct v::uuid) into endpoints from jsonb_array_elements_text(jsonb_build_array(
   p_row->>'from_claim_id',p_row->>'to_claim_id',p_row->>'claim_id',p_row->>'target_claim_id')) v where v is not null;
 if cardinality(endpoints)>0 then
   select array_agg(distinct subject_id),count(*) into ids,endpoint_count
   from public.semantic_claims where user_id=p_owner and id=any(endpoints)
     and subject_type='pet' and subject_id is not null;
   if endpoint_count=cardinality(endpoints) then return ids; end if;
 end if;
 -- A retained care link alone cannot disambiguate a missing claim's subject.
 return coalesce((select array_agg(id order by id) from public.dog_profiles where user_id=p_owner),'{}');
end $$;
revoke all on function private.ask_removed_pet_scope(uuid,jsonb) from public,anon,authenticated,service_role;
update public.ask_recorded_inventory_removals r set affected_pet_ids=private.ask_removed_pet_scope(r.user_id,'{}');
update public.ask_history_removed_relation_targets r set affected_pet_ids=private.ask_removed_pet_scope(r.user_id,to_jsonb(r));

create function private.capture_ask_removed_pet_scope() returns trigger
language plpgsql security definer set search_path=pg_catalog as $$
begin
 new.affected_pet_ids := private.ask_removed_pet_scope(new.user_id,to_jsonb(new));
 return new;
end $$;
revoke all on function private.capture_ask_removed_pet_scope() from public,anon,authenticated,service_role;
create trigger capture_ask_removed_pet_scope before insert on public.ask_history_removed_relation_targets
 for each row execute function private.capture_ask_removed_pet_scope();
...
begin
 ...
 if tg_op='DELETE' and tg_table_name in ('pet_care_entries','pet_care_episodes','pet_care_episode_events','semantic_claims','semantic_claim_relations','semantic_claim_legacy_lineage','ask_history_removed_relation_targets','ask_recorded_source_evidence') then
   insert into public.ask_recorded_inventory_removals(user_id,affected_pet_ids)
     select id,private.ask_removed_pet_scope(id,to_jsonb(old)) from auth.users where id=(to_jsonb(old)->>'user_id')::uuid
   on conflict(user_id) do update set affected_pet_ids=array(
     select distinct p from unnest(public.ask_recorded_inventory_removals.affected_pet_ids || excluded.affected_pet_ids) p order by p);
 end if;
...
returns jsonb language sql stable security definer set search_path=pg_catalog as $$
 ...
 select case when
   ...
   and not exists(select 1 from public.semantic_claim_relations r where r.user_id=auth.uid()
     and r.relation_type in ('corrects','supersedes','retracts') and exists(
       select 1 from public.semantic_claims c where c.user_id=r.user_id and c.id in (r.from_claim_id,r.to_claim_id)
         and (c.subject_type<>'pet' or c.subject_id is null or c.subject_id=p_pet_id)))
   and not exists(select 1 from public.ask_recorded_inventory_removals where user_id=auth.uid() and p_pet_id=any(affected_pet_ids))
   and not exists(select 1 from public.ask_history_removed_relation_targets where user_id=auth.uid() and p_pet_id=any(affected_pet_ids))
...
begin
 ...
 if exists(select 1 from public.ask_recorded_inventory_removals where user_id=auth.uid() and p_pet_id=any(affected_pet_ids)) then
