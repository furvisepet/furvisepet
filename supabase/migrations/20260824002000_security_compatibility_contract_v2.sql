...
begin
  ...
  -- Reuse V1's already-deployed semantic checks without depending on source
  -- migration timestamps. Passing an actually recorded version makes V1 check
  -- the live authority contract rather than an environment-specific filename.
  ...
  -- Old and renamed canonical-care writers must be unreachable through every
  -- browser/service API role. Trusted server wrappers execute them as owner.
  v_ok := true;
  ...
  -- The three application-facing canonical writers are service-only,
  -- SECURITY DEFINER, and must delegate through the server-actor guard.
  foreach v_signature in array array[
