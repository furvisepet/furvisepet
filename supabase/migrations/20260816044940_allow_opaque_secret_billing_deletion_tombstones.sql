...
-- The opaque sb_secret key is authorized by the API gateway as the service_role
-- database role, but it is not itself a legacy JWT and does not populate the
-- legacy JWT role-claim setting. EXECUTE remains service-role-only.
