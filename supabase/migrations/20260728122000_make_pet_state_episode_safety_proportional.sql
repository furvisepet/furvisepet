-- Active food, medication, and routine episodes are useful context, not urgent safety state.
...
declare
  ...
  v_active uuid[]; v_monitoring uuid[]; v_sources uuid[]; v_status text; v_overall text; v_active_safety boolean;
begin
  ...
  select exists(select 1 from public.pet_care_episodes where user_id = v_owner and pet_profile_id = p_pet_id and status = 'active' and episode_type = 'symptom') into v_active_safety;
  ...
  v_overall := case when v_status = 'abnormal' or v_active_safety then 'urgent' when v_status = 'normal' then 'monitoring' else 'uncertain' end;
...
declare
  ...
  v_previous public.pet_current_state%rowtype; v_state jsonb; v_active uuid[]; v_monitoring uuid[];
  v_breathing_status text; v_overall text; v_active_safety boolean;
begin
  ...
  if v_text ~* '(breath|breathing)' then
    v_breathing_status := case when new.state_action_type = 'resolve_concern' or v_text ~* '(returned to normal|back to normal|normal again|breathing is normal)' then 'normal'
      when new.state_action_type = 'reopen_concern' or new.severity in ('moderate', 'severe') or v_text ~* '(difficult|deep breaths?|trouble breathing|recurred|problem.*back)' then 'abnormal' else 'uncertain' end;
    v_state := jsonb_set(v_state, '{breathing}', jsonb_build_object('status', v_breathing_status, 'confidence', coalesce(new.intelligence_confidence, 1), 'lastObservedAt', new.occurred_at, 'sourceEventId', new.id));
  else v_breathing_status := v_state#>>'{breathing,status}'; end if;
  select coalesce(array_agg(id order by last_event_at), '{}') into v_active from public.pet_care_episodes where user_id = new.user_id and pet_profile_id = new.pet_profile_id and status = 'active';
  select coalesce(array_agg(id order by last_event_at), '{}') into v_monitoring from public.pet_care_episodes where user_id = new.user_id and pet_profile_id = new.pet_profile_id and status = 'monitoring';
  select exists(select 1 from public.pet_care_episodes where user_id = new.user_id and pet_profile_id = new.pet_profile_id and status = 'active' and episode_type = 'symptom') into v_active_safety;
  v_overall := case when v_breathing_status = 'abnormal' or v_active_safety then 'urgent' when v_breathing_status = 'normal' then 'monitoring' else coalesce(v_state#>>'{wellbeing,overall}', 'uncertain') end;
  v_state := jsonb_set(jsonb_set(v_state, '{wellbeing}', jsonb_build_object('overall', v_overall)), '{lastMeaningfulUpdateAt}', to_jsonb(new.occurred_at));
  insert into public.pet_current_state(pet_profile_id,user_id,state_version,state,active_episode_ids,monitoring_episode_ids,source_event_ids,computed_at,updated_at)
  values(new.pet_profile_id,new.user_id,1,v_state,v_active,v_monitoring,array[new.id],now(),now())
  on conflict(pet_profile_id) do update set state_version=public.pet_current_state.state_version+1,state=excluded.state,active_episode_ids=excluded.active_episode_ids,
    monitoring_episode_ids=excluded.monitoring_episode_ids,source_event_ids=case when new.id=any(public.pet_current_state.source_event_ids) then public.pet_current_state.source_event_ids else public.pet_current_state.source_event_ids||new.id end,computed_at=now(),updated_at=now();
  return new;
end;
$$;
