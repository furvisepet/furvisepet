...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'dog_product_feedback_type_check'
      and conrelid = 'public.dog_product_feedback'::regclass
  ) then
    alter table public.dog_product_feedback
      add constraint dog_product_feedback_type_check
      check (
        feedback_type in (
          'saved',
          'tried',
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'pet_care_entries_category_check'
      and conrelid = 'public.pet_care_entries'::regclass
  ) then
    alter table public.pet_care_entries
      add constraint pet_care_entries_category_check
      check (category in ('symptom', 'food', 'medication', 'activity', 'grooming', 'vet_visit', 'behavior', 'general'));
  end if;
end $$;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'pet_care_entries_severity_check'
      and conrelid = 'public.pet_care_entries'::regclass
  ) then
    alter table public.pet_care_entries
      add constraint pet_care_entries_severity_check
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'pet_care_entries_note_check'
      and conrelid = 'public.pet_care_entries'::regclass
  ) then
    alter table public.pet_care_entries
      add constraint pet_care_entries_note_check
      check (btrim(note) <> '');
  end if;
end $$;
