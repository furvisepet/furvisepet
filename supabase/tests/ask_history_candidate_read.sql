...
create function pg_temp.rpc(c cases) returns text language sql as $$
 select format('select * from public.read_ask_history_candidates(%L::uuid,%L::text[],%L::timestamptz,%L::timestamptz,%L::timestamptz,%L::uuid,25)',c.pet,c.terms,c.lo,c.hi,c.ct,c.ci)
$$;
...
do $$ declare seen uuid[]:='{}'; last_time timestamptz; last_id uuid; r record; n int; pages int:=0; begin
 loop
  n:=0;
  for r in select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit','threw up','thrown up'],null,null,last_time,last_id,25) loop
...
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('bad',array['vomit'])$q$,'22P02');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'],'bad','2015-01-01')$q$,'22007');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000013',array['vomit'])$q$,'42501');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000099',array['vomit'])$q$,'42501');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates(null,array['vomit'])$q$,'42501');
do $$ declare terms text[]; lim int; begin
 foreach terms slice 1 in array array[array[''],array['ab'],array['%vomit%'],array['vo_mit'],array['vomit;drop table'],array[repeat('a',33)],array[null::text]] loop
  perform pg_temp.expect_error(format('select * from public.read_ask_history_candidates(%L,%L::text[])','93000000-0000-4000-8000-000000000011',terms),'22023');
 end loop;
 foreach lim in array array[0,26,100000,-1,null::int] loop
  perform pg_temp.expect_error(format('select * from public.read_ask_history_candidates(%L,array[''vomit''],null,null,null,null,%L)','93000000-0000-4000-8000-000000000011',lim),'22023');
 end loop;
end $$;
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',null)$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011','{}')$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array_fill('vomit'::text,array[7]))$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array[['vomit','stool']])$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011','[0:0]={vomit}')$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'],'2015-01-01',null)$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'],'2015-01-01','2014-01-01')$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'],'-infinity','infinity')$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'],null,null,'2014-01-01',null)$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'],null,null,null,'95000000-0000-4000-8000-000000000061')$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'],'2014-01-01','2015-01-01','2015-01-01','95000000-0000-4000-8000-000000000061')$q$,'22023');
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'],null,null,'infinity','95000000-0000-4000-8000-000000000061')$q$,'22023');
do $$ begin
 if (select count(*) from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit','threw up','thrown up','stool','weight','rice'],null,null,null,null,1))<>1 then raise exception 'Valid bounds rejected'; end if;
end $$;
select set_config('request.jwt.claim.sub','',true);
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'])$q$,'42501');
reset role;
set local role anon;
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'])$q$,'42501');
reset role;
...
set local role authenticated;
select set_config('request.jwt.claim.sub','93000000-0000-4000-8000-000000000001',true);
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000012',array['grooming'])$q$,'42501');
select set_config('request.jwt.claim.sub','93000000-0000-4000-8000-000000000002',true);
do $$ begin
 if exists(select 1 from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000012',array['grooming'])) then raise exception 'Transfer leaked stale-owner data'; end if;
 if (select count(*) from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000013',array['vomit']))<>25 then raise exception 'Second owner positive control'; end if;
...
select pg_temp.expect_error($q$select * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array['vomit'])$q$,'55000');
...
do $$ declare term text; begin
 foreach term in array array['vomit','threw up','thrown up','stool','diarrh','weight','weigh','food','rice','diet','litter','medication','stiff','urine','urinalysis','blood','diagnos'] loop
  perform * from public.read_ask_history_candidates('93000000-0000-4000-8000-000000000011',array[term]);
 end loop;
 raise notice 'PASS: every current planner term accepted by actual RPC';
end $$;
reset role;
do $$ declare fn regprocedure := 'public.read_ask_history_candidates(uuid,text[],timestamptz,timestamptz,timestamptz,uuid,integer)'::regprocedure; begin
