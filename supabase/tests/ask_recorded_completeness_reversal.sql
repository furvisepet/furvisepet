...
begin;
create temp table recorded_function_before as select pg_get_functiondef('public.read_ask_episode_sources(uuid,text[],uuid[],timestamptz,timestamptz)'::regprocedure) as body;
...
do $$ begin
 if position('recorded_inventory' in pg_get_functiondef('public.read_ask_episode_sources(uuid,text[],uuid[],timestamptz,timestamptz)'::regprocedure))>0 then raise exception 'reader not reversed'; end if;
 if to_regclass('public.ask_recorded_inventory_revision') is not null then raise exception 'revision not reversed'; end if;
 if has_function_privilege('anon','public.read_ask_episode_sources(uuid,text[],uuid[],timestamptz,timestamptz)','execute') then raise exception 'anon grant'; end if;
end $$;
rollback;
