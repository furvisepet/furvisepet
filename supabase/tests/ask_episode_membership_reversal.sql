-- NOT DATABASE-VALIDATED. Requires the draft installed in a disposable DB.
-- Exercise the exact reversal body inside an outer rollback-only transaction.
begin;
create temporary table membership_function_before as select pg_get_functiondef(
 'public.read_ask_episode_sources(uuid,text[],uuid[],timestamptz,timestamptz)'::regprocedure) definition;
do $$ begin
 if not (select definition like '%ask-episode-membership.v1%' from membership_function_before) then
  raise exception 'Membership draft must be installed before reversal test';
 end if;
end $$;
...
do $$ declare definition text; begin
 select pg_get_functiondef('public.read_ask_episode_sources(uuid,text[],uuid[],timestamptz,timestamptz)'::regprocedure) into definition;
 if definition like '%ask-episode-membership.v1%' or definition not like '%FROM public.pet_care_entries%'
   and definition not like '%from public.pet_care_entries%' then raise exception 'Prior reader not restored'; end if;
 if (select prosecdef from pg_proc where oid='public.read_ask_episode_sources(uuid,text[],uuid[],timestamptz,timestamptz)'::regprocedure) then
  raise exception 'Reversal must restore security invoker'; end if;
 if has_function_privilege('anon','public.read_ask_episode_sources(uuid,text[],uuid[],timestamptz,timestamptz)','execute')
  or not has_function_privilege('authenticated','public.read_ask_episode_sources(uuid,text[],uuid[],timestamptz,timestamptz)','execute') then
  raise exception 'Reversal grants changed'; end if;
end $$;
rollback;