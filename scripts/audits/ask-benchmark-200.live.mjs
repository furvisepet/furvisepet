...
assert.ok(!existsSync(resultPath)&&!existsSync(budgetPath),'Never overwrite/retry a previous benchmark');
const budget={authorizedUsd:5,stopUsd:4.5,priorMeasuredUsd,priorCalls:previous.calls.length,pricing:{inputPerMillion:.75,outputPerMillion:4.5},calls:[]};
const output={baseline,datasetSha256,model,syntheticDatabase:true,realProviders:true,httpRoute:false,clock:'2026-09-15T12:00:00Z',startedAt:new Date().toISOString(),turns:[]};
...
save();
for(const q of [...questions].sort((a,b)=>(a.id-1)%10-(b.id-1)%10||a.id-b.id)){
