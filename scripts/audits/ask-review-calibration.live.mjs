...
const output={realReviewer:true,mockedInterpretationAndDraft:true,results:[]};
const file=new URL('../../docs/ask-review-calibration.json',import.meta.url);if(existsSync(file))throw Error('Already captured');
console.info=()=>{};
mock.timers.enable({apis:['Date'],now:Date.parse(ledger.syntheticClock)});
for(const [id,draft] of [['supported',valid],['unsupported',valid.replace('The cause of any change is unknown.','A veterinarian diagnosed incurable cancer, causing the travel changes.')]]){
 ...
 const item={id,answer:r.result.reasoning.answer.summary,review:ledger.calls.at(-1)?.review,rationale:ledger.calls.at(-1)?.rationale};output.results.push(item);console.log(JSON.stringify(item));writeFileSync(file,JSON.stringify(output,null,2)+'\n');
}
mock.timers.reset();
