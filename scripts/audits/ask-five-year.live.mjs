...
const ledgerPath=new URL('../../docs/ask-five-year-budget.json',import.meta.url);
const ledger=existsSync(ledgerPath)?JSON.parse(readFileSync(ledgerPath,'utf8')):{authorizedUsd:15,reservationStopUsd:4,priorAdditionalCommittedUsd:10.979268750000001,reservedUsd:0,measuredUsd:0,calls:[],pricingSource:'https://developers.openai.com/api/docs/models/gpt-5.4-mini',pricing:{inputPerMillion:0.75,outputPerMillion:4.5},syntheticClock:'2026-09-09T12:00:00Z'};
...
async function liveCall(request,options={}) {
  ...
  if(ledger.calls.length>=1000||ledger.reservedUsd+worst>4) throw new Error('LOCAL_TEST_BUDGET_STOP');
...
const output={syntheticDatabase:true,realProviders:true,productionRoute:false,productionBrowser:false,fixtureFile,fixtureRows:rows.length,phase,turns:[]};let messages=[];
