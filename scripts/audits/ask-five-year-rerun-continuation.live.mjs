...
const ledgerPath=new URL('../../docs/ask-five-year-rerun-continuation-budget.json',import.meta.url);
if(existsSync(ledgerPath))throw Error('Refusing to overwrite rerun ledger');
const ledger={authorizedUsd:3.2685,priorNewAllowanceSpendUsd:0.385623,reservedUsd:0,measuredUsd:0,calls:[],pricing:{inputPerMillion:0.75,outputPerMillion:4.5},syntheticClock:'2026-09-09T12:00:00Z'};
...
if(selectedIds)throw Error('Continuation uses fixed remaining range');
const scenarios=allScenarios.filter(x=>x.id>=67);
const phase='rerun200-continuation';
...
if(process.argv.includes('--dry-run')){assert.equal(rows.length,1169);assert.equal(scenarios.length,134);console.log(JSON.stringify({dryRun:true,cases:scenarios.length,rows:rows.length,model,cap:ledger.authorizedUsd}));process.exit(0);}
