...
const sourceDir=new URL('../../docs/ask-benchmark-200/',import.meta.url);
const dir=new URL('../../tmp/ask-v1-benchmark/',import.meta.url);mkdirSync(dir,{recursive:true});
const runName=process.argv.find(a=>a.startsWith('--name='))?.slice(7)||'development-1';assert.match(runName,/^[a-z0-9-]+$/);
const ids=(process.argv.find(a=>a.startsWith('--ids='))?.slice(6)||'').split(',').map(Number).filter(Boolean);
const questions=JSON.parse(readFileSync(new URL('questions.json',sourceDir),'utf8'));
const datasetSha256=createHash('sha256').update(readFileSync(new URL('questions.json',sourceDir))).digest('hex');
...
const workingDiffSha256=createHash('sha256').update(execFileSync('git',['diff'])).digest('hex');
const priorMeasuredUsd=0;
...
const resultPath=new URL(runName+'.json',dir),budgetPath=new URL('budget.json',dir);
assert.ok(!existsSync(resultPath),'Never overwrite/retry a previous benchmark');
const budget=existsSync(budgetPath)?JSON.parse(readFileSync(budgetPath,'utf8')):{authorizedUsd:5,stopUsd:4.8,priorMeasuredUsd:0,pricing:{inputPerMillion:.75,outputPerMillion:4.5},calls:[]};
const output={baseline,workingDiffSha256,runName,datasetSha256,model,syntheticDatabase:true,realProviders:true,httpRoute:false,clock:'2026-09-15T12:00:00Z',startedAt:new Date().toISOString(),turns:[]};
...
async function liveCall(request,options={}) {
 ...
 const call={id:budget.calls.length+1,caseId:currentId,runName,phase:request.text?.format?.name,reservedUsd:worst,status:'reserved'};
...
mock.timers.reset();output.finishedAt=new Date().toISOString();output.completed=output.turns.length===questions.filter(q=>!ids.length||ids.includes(q.id)).length;save();
