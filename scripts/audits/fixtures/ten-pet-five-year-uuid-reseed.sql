-- Supersedes identifier-only repair: derived episode references require atomic fixture regeneration.
-- Requires explicitly authorized untouched synthetic account; source facts verified unchanged.
begin;
create temporary table fixture_content_before as select category,title,note,severity,occurred_at,created_at,care_event_metadata from public.pet_care_entries where user_id=current_setting('furvise.test_account_id')::uuid;
do $check$ begin
if (select count(*) from fixture_content_before)<>3729 or exists(select 1 from fixture_content_before where care_event_metadata->>'dataset' is distinct from 'furvise-ten-pets-five-years-20260909-v1') or exists(select 1 from public.ask_conversations where user_id=current_setting('furvise.test_account_id')::uuid) then raise exception 'Not untouched synthetic fixture'; end if;
end $check$;
create function pg_temp.fixture_uuid(input text) returns uuid language sql immutable as $$ select overlay(overlay(md5(input) placing '4' from 13 for 1) placing '8' from 17 for 1)::uuid $$;
delete from public.dog_profiles where user_id=current_setting('furvise.test_account_id')::uuid;
...
begin
 ...
 for cfg in select value from jsonb_array_elements($config$[{"name":"Juniper","species":"dog","breed":"Border Collie mix","sex":"female","age":7,"weight":18.6,"food1":"chicken-and-rice adult dry food","food2":"lamb-and-oat adult dry food","activity":"walk","minutes":38},{"name":"Pixel","species":"cat","breed":"Domestic shorthair","sex":"male","age":6,"weight":4.9,"food1":"turkey complete adult wet food","food2":"chicken complete adult wet food","activity":"wand-toy play","minutes":14},{"name":"Atlas","species":"dog","breed":"Golden Retriever","sex":"male","age":9,"weight":31.2,"food1":"salmon-and-rice adult dry food","food2":"turkey senior dry food","activity":"park walk","minutes":32},{"name":"Mochi","species":"cat","breed":"Calico domestic shorthair","sex":"female","age":8,"weight":3.8,"food1":"chicken complete adult wet food","food2":"rabbit complete adult wet food","activity":"puzzle-feeder play","minutes":11},{"name":"Clover","species":"other","breed":"Rabbit — Mini Lop","sex":"female","age":6,"weight":2.1,"food1":"timothy hay with measured plain pellets and leafy greens","food2":"orchard hay with measured plain pellets and leafy greens","activity":"supervised indoor exploration","minutes":42},{"name":"Ziggy","species":"dog","breed":"Terrier mix","sex":"male","age":7,"weight":9.4,"food1":"beef-and-barley adult dry food","food2":"whitefish-and-potato adult dry food","activity":"sniff walk","minutes":26},{"name":"Nori","species":"cat","breed":"Siamese mix","sex":"female","age":10,"weight":4.2,"food1":"whitefish complete adult wet food","food2":"turkey complete adult wet food","activity":"feather-toy play","minutes":9},{"name":"Pebble","species":"other","breed":"Rabbit — Rex mix","sex":"male","age":7,"weight":2.8,"food1":"meadow hay with measured plain pellets and leafy greens","food2":"timothy hay with measured plain pellets and leafy greens","activity":"supervised pen exploration","minutes":35},{"name":"Maple","species":"dog","breed":"Beagle","sex":"female","age":8,"weight":12.7,"food1":"turkey-and-oat adult dry food","food2":"lamb-and-rice adult dry food","activity":"neighborhood walk","minutes":29},{"name":"Cosmo","species":"cat","breed":"Maine Coon mix","sex":"male","age":6,"weight":6.6,"food1":"beef complete adult wet food","food2":"salmon complete adult wet food","activity":"climbing-and-toy play","minutes":17}]$config$::jsonb) loop
  i:=i+1; p:=pg_temp.fixture_uuid(ds||':pet:'||i)::uuid; switch_month:=20+(i*7)%24;
  ...
  for m in 0..59 loop
   for k in 0..5 loop
    ...
    values(pg_temp.fixture_uuid(ds||':'||i||':'||m||':'||k)::uuid,u,p,cat,'Synthetic history — '||label,n,d+time '12:00',d+time '18:00',jsonb_build_object('synthetic',true,'dataset',ds,'sequence',m*6+k));
    expected:=expected+1;
   end loop;
  end loop;
  ...
  for y in 0..4 loop
   ...
   for k in 0..4 loop
    ...
    values(pg_temp.fixture_uuid(ds||':annual:'||i||':'||y||':'||k)::uuid,u,p,cat,'Synthetic history — '||label,n,case when k=1 then 'mild' else null end,
  ...
  for k in 0..5 loop
   ...
   insert into public.pet_care_entries(id,user_id,pet_profile_id,category,title,note,occurred_at,created_at,care_event_metadata)
   values(pg_temp.fixture_uuid(ds||':milestone:'||i||':'||k)::uuid,u,p,cat,'Synthetic history — '||label,n,d+time '12:00',d+time '18:00',jsonb_build_object('synthetic',true,'dataset',ds));
   expected:=expected+1;
  end loop;
  insert into public.pet_care_entries(id,user_id,pet_profile_id,category,title,note,occurred_at,created_at,care_event_metadata)
  values(pg_temp.fixture_uuid(ds||':latest:'||i)::uuid,u,p,'general','Synthetic history — Latest profile measurement',
...
do $check$ begin
if exists((select category,title,note,severity,occurred_at,created_at,care_event_metadata from public.pet_care_entries where user_id=current_setting('furvise.test_account_id')::uuid except all select * from fixture_content_before)) or exists((select * from fixture_content_before except all select category,title,note,severity,occurred_at,created_at,care_event_metadata from public.pet_care_entries where user_id=current_setting('furvise.test_account_id')::uuid)) then raise exception 'Fixture facts changed'; end if;
end $check$;
commit;