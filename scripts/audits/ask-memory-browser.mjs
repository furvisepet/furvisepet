...
function bundle(file) {
  ...
  if (/\.[mt]sx?$/.test(file) || file === resolve('tests/fixtures/ask-memory-browser.js')) source = ts.transpileModule(source, { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022, jsx: ts.JsxEmit.ReactJSX } }).outputText;
...
const entry = bundle('tests/fixtures/ask-memory-browser.js');
