...
async function fetchDraft(petId: string, from: string, to: string, conversationId: string, existingDocument?: VetBriefDocument | null) { return authenticatedJson("/api/vet-briefs/draft", { method: "POST", body: JSON.stringify({ conversationId: conversationId || undefined, existingDocument: existingDocument || undefined, from, petId, requestId: crypto.randomUUID(), to }) }) as Promise<{ document: VetBriefDocument; sourceEntryIds: string[] }>; }
async function authenticatedJson(url: string, init: RequestInit = {}) { const token = await getAuthToken(); if (!token) throw new Error("Please sign in again."); const response = await fetch(url, { ...init, headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json", ...(init.headers || {}) } }); const payload = await response.json().catch(() => null) as { error?: string } | null; if (!response.ok) throw new Error(payload?.error || "The Vet Visit Brief is temporarily unavailable."); return payload; }
