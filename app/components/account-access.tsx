    ...
    <main className="min-h-screen bg-[var(--surface-page)] text-[var(--text-primary)]">
      <PageShell className="flex min-h-screen flex-col" preset="reading">
        ...
        <section className="flex flex-1 items-center justify-center py-8 sm:py-12">
          <div className="w-full max-w-[480px] rounded-3xl border border-[var(--line)] bg-[var(--surface-primary)] p-6 shadow-[var(--shadow-surface-1)] sm:p-8">
            <h1 className="text-[2.55rem] font-semibold leading-[1.04] tracking-[-0.035em] sm:text-[3.15rem]">{title}</h1>
...
export function AccountStatus({
  ...
  return <div className={`border-y py-3 text-sm font-medium leading-6 ${toneClasses}`} role="status">{text}</div>;
}
