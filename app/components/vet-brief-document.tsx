...
export function VetBriefDocumentView({ document, version }: { document: VetBriefDocument; version?: number }) {
  ...
  return (
    <article className="vet-brief-document mx-auto w-full max-w-[8.5in] bg-[var(--pw-document-paper)] px-6 py-8 text-[var(--pw-document-text)] sm:px-12 sm:py-12 print:max-w-none print:px-[0.65in] print:py-[0.55in]">
      <header className="border-b-2 border-[var(--pw-document-brand)] pb-6">
        ...
        <p className="mt-4 text-xs leading-5 text-[var(--pw-document-muted)]">Prepared {formatDate(document.generatedAt.slice(0, 10))}. Selected records: {formatDate(document.dateRange.from)}–{formatDate(document.dateRange.to)}.<br />Owner-recorded history; this is not a complete medical record.</p>
      </header>
      {sections.map(section => <section className="border-b border-[var(--pw-document-border-soft)] py-5" key={section.id}>
        <h2 className="mb-3 break-after-avoid text-sm font-bold text-[var(--pw-document-brand)]">{section.title}</h2>
        <ul className="space-y-4">{section.items.map((item, index) => <li className="break-inside-avoid text-[0.95rem] leading-6" key={index}>
