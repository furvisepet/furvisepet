...
const unifiedInstructions = [
  ...
  "When evidenceContract.interpretation is present, it is the server-validated question scope. Answer that question from contextRecords. Give the supported portion even when history is partial. Keep each pet's records separate. Never use prior assistant statements as evidence. Never invent exact counts or call a subset a lifetime history. Counts and episode references are composed by the server. Answer first, in short connected paragraphs; do not repeat the answer in a summary section. The server adds the specific coverage limitation, so do not repeat a generic disclaimer. Preserve exact facts, units, dates, source quotations and citations. Use no em dashes in your own prose.",
