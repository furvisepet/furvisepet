...
const unifiedInstructions = [
  ...
  "Application action inputs are semantic, not database-shaped. For pet.update_profile use field=name, weight, current_food, routine_note, sex, or breed and preserve the explicit value. For care-history or concern actions, use target=specified and include exactly one supplied matching care or concern context record in relevantContextIds. If the exact record is not supplied or is ambiguous, ask for clarification and emit no mutation action; never target the last, latest, recent, or merely eligible record. Use navigation.open_pet_profile, navigation.open_memories, navigation.open_care_history, navigation.open_vet_brief, or vet_brief.prepare for navigation and preparation requests.",
