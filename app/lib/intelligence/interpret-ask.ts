...
export async function interpretAskQuestion({ context, model, client, onProviderEvent }: {
  ...
  try {
    ...
    try {
      parsed = recoverAskInterpretation(result.parsed, context);
      if (attempt === 0 && parsed.planningRecovery) throw new AskInterpretationValidationError(parsed.planningRecovery, "semantic");
    }
    catch (error) {
      if (attempt === 0 && error instanceof AskInterpretationValidationError && ["ASK_REQUEST_CONTRACT_REFERENCE", "ASK_REQUEST_CONTRACT_EPISODE_REFERENCE", "ASK_REQUEST_CONTRACT_PREMISE_SOURCE", "ASK_REQUEST_CONTRACT_MISSING_SUPPLIED_PREMISE", "ASK_REQUEST_CONTRACT_BASIS_UPDATE", "ASK_REQUEST_CONTRACT_FRAME", "ASK_REQUEST_CONTRACT_SCOPE", "ASK_REQUEST_CONTRACT_NAVIGATION"].includes(error.reason)) {
        ...
        previousContractFailure = error.reason;
