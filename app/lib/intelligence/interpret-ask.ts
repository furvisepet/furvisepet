...
export async function interpretAskQuestion({ context, model, client, onProviderEvent }: {
  ...
  try {
    ...
    try { parsed = recoverAskInterpretation(result.parsed, context); }
      ...
      if (attempt === 0 && error instanceof AskInterpretationValidationError && ["ASK_REQUEST_CONTRACT_PREMISE_SOURCE", "ASK_REQUEST_CONTRACT_MISSING_SUPPLIED_PREMISE", "ASK_REQUEST_CONTRACT_BASIS_UPDATE"].includes(error.reason)) {
