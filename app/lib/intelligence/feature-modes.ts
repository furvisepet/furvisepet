...
const modes = {
  ...
  vet_brief: mode("vet_brief", "vet_brief", intelligenceVetBriefJsonSchema, "furvise_vet_brief", {
    ...
    instructions: [
      "Complete the supplied documentTemplate using relevant recorded source facts. Its empty detail arrays are intentional, not evidence that history is absent. Live-context records are candidates retrieved for the date range, not a pre-approved list of relevant facts. Select the records that inform the explicit visit purpose before writing. Copy immutable profile and date-range fields from the template.",
      ...
      "visitSummary is a short evidence-backed synopsis, at most three sentences: the main concern, its documented course, and latest relevant recorded observation or important uncertainty. Lead with the actual evidence, not a statement that this is appointment preparation or a restatement of the request. When a comparison is requested, put both dated or current values together in the synopsis and distinguish their time scopes. Include what was tried and the recorded response when relevant and supported. It should save the veterinarian reading time. Do not invent onset, current status, diagnosis, or causation. For sparse records say what is available without overstating its usefulness. The synopsis may refer to key facts detailed in the timeline.",
      "Preserve material symptom progression, resolution, medication records and relevant visits. Every retained detail must inform the explicit appointment concern or a requested comparison. Omit unrelated routine housekeeping, grooming and enrichment rather than cataloguing all available records. Only when the owner gives no narrower concern or comparison may a general routine appointment summarize ordinary observations compactly. Words such as routine or checkup do not broaden an explicitly focused request. Do not include an unrelated event merely to say it does not explain the concern. Do not omit safety-relevant history to meet a length target.",
      ...
      "If reviewRepair is supplied, revise its document to satisfy every failed check while preserving relevant supported facts. Remove irrelevant facts even when they are true, and do not restore unrelated baseline entries during repair. The repaired document will be reviewed independently again.",
