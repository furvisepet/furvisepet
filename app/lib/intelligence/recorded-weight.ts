...
export function recordedWeightGrams(note: string, petName: string): number | null {
 ...
 if(!match && new RegExp(`^(?:Note: )?${escaped}\\b`).test(note)) {
   const pronoun=/(?:^|\.\s+)(?:His|Her|Their) weight today was ([0-9]{1,6})(?:\.([0-9]{1,3}))? kg\.(?:\s|$)/.exec(note);
   if(pronoun) {
     ...
     if(names.every(name=>[petName,'Note','His','Her','Their'].includes(name))
       && !/\b(?:other|another|dog|cat|pet|friend|sister|brother|owner|mother|father)\b/i.test(prefix)) {
