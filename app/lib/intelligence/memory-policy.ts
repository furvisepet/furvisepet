...
export function evaluateLearningPolicy(learnings: IntelligenceLearning[], currentMessage: string, petId: string) {
  ...
  for (const learning of learnings.slice(0, 8)) {
    const reason = rejectLearningReason(learning, currentMessage, petId);
    if (reason) rejected.push({ learning, reason }); else accepted.push({ ...learning, subjectId: learning.subjectType === "pet" ? petId : null });
  }
...
function rejectLearningReason(learning: IntelligenceLearning, currentMessage: string, petId: string) {
  ...
  if (learning.subjectType === "pet" && learning.subjectId && learning.subjectId !== petId) return "wrong_pet";
