...
export function parseEpisodeFollowUp(message: string): { ordinal: string; ambiguous: boolean } | null {
  const match = /\b(first|second|third|fourth|fifth|sixth|seventh|eighth|last|that)\s+(?:(?:vomiting|soft[- ]stool|stool|diarrhea|breathing)\s+)?(?:one|episodes?)\b/i.exec(message);
  if (!match) return null;
  const ordinals = message.match(/\b(?:first|second|third|fourth|fifth|sixth|seventh|eighth|last)\b/gi) || [];
  const demonstratives = message.match(/\bthat\s+(?:(?:vomiting|soft[- ]stool|stool|diarrhea|breathing)\s+)?(?:one|episodes?)\b/gi) || [];
  return { ordinal: match[1].toLowerCase(), ambiguous: ordinals.length + demonstratives.length !== 1 || /\bepisodes$/i.test(match[0]) };
}
