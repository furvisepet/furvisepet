...
Ask authenticates, verifies pet ownership, persists/reuses the user message, and loads profile, Pet State, active concerns, bounded episodes/events, fresh memories, and recent conversation in parallel. Deterministic safety runs before provider admission. An admitted conversational turn first requests a structured question interpretation (including a subject frame for owner updates). The server validates ownership, scope, dates, operations and query budgets, then retrieves historical evidence and requests the answer. Both requests share the existing two-call ceiling and one reserved user credit. Governance and answer validation run after generation. Approved canonical changes commit before the assistant response claims persistence. The assistant answer is then persisted and exactly one reserved credit is finalized. Validation or provider failure releases the reservation and keeps the user message retryable.

The validated Ask interpretation is carried on the live context through the actual history callback, episode reader and evidence contract. Legacy direct callers retain deterministic compatibility; the production read flow does not independently reinterpret the question at each retrieval stage. Owner updates still use the existing grounded subject and write governance. The interpretation cannot authorize a write. Older lexical/dated records remain reachable through the existing bounded candidate and correction readers; an exhausted cursor does not certify lifetime completeness. The requested read operation is independent of current owner assertions: mixed observations and historical comparisons retrieve evidence while every write remains source-grounded separately. Explicit selection intent controls retained history and prompt order, including temporal boundaries and visible pet attribution. Interpreted historical synthesis is validated per complete source using bounded equivalent forms; unsupported proposals fall back individually. A model source ID does not prove arbitrary narrative entailment. Existing server episode computations remain the count authority. Final presentation may restore only the completed callback's validated answer, after episode revalidation, and must preserve downstream safety/governance changes. See `ask-conversational-v1-checkpoint.md` for validation scope and remaining limits.
