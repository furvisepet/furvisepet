...
The readiness route is not public: send `X-Furvise-Operator-Key` through deployment-protected access. It checks safe configuration state, database connectivity, the explicit application/security migration contract, live security-authority capabilities, and Redis with bounded timeouts. The service-only compatibility probe verifies high-value grants, RLS/trigger state, required RPC signatures, and action freshness/expiry columns; it reads catalogs but never repairs schema. The response remains generic and never returns IDs, URLs, keys, migration versions, table names, or failed probe details. Safe compatibility codes are emitted only to internal operational diagnostics.
