...
| Supabase Postgres/functions | database operator | DB password, secret key, management token | readiness DB unavailable, schema incompatibility, integrity findings | stop writes/AI, restore isolated copy, forward-fix | all canonical user data and migration history | explicit security-migration set plus live compatibility probes after apply |
...
| Migrations | database operator | linked CLI/DB credentials | parity/schema-capability mismatch or readiness failure | stop deployment; investigate before writes | schema migration history | CLI parity and protected readiness required per deploy |
